...
\begin{frame}{DP Algorithm}
	The Dynamic Programming(DP) Algorithm to solve separation problem.
	\begin{description}
	\justifying
	\footnotesize
...
\begin{frame}{Machine Scheduling Game with Weighted Jobs}
	\begin{itemize}
	\justifying
		\item ${\omega(P)}$ is piecewise linear, and the slope is $m(V)-\sum_{s\in S\setminus\{V\}}\rho_sm(S)$.
		\item $c(S)$ is not so easy to obtain.
		\item $P_m = c_0(V,m)- c_0(V,m-1), 2 \leq m \leq v$.
		\item Separation problem can be solved in pseudo-polynomial time.
\end{itemize}
\end{frame}
...
\begin{frame}{Pricing in General IM Games}
	...
	\justifying
		...
		\item $c_0 (V,m) - c_0 (V,m+1) < c_0 (V,m-1) - c_0 (V,m) \Leftrightarrow \quad P_m < P_{m+1} , m=2,3,\ldots,n.$
	\end{itemize}
\end{frame}
